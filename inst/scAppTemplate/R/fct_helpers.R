...
plot_prep_server <- function(
    ...
    ## Note: plot legend in separate plot ##
    ...
    plotInput <- reactive({
      ...
      if (is.numeric(df$x_axis)){
          ...
          if (df$y_axis[1] == "Densityplot"){
              ...
              p <- ggplot2::ggplot(
                #data = df, ggplot2::aes_string(x=input$x_axis, color=input$colorBy,fill=input$colorBy)
                ...
                p <- ggplot2::ggplot(
                    data = df, ggplot2::aes_string(x=x_axis, y=y_axis, color=colorBy)
        ...
        plotLogic <- "violin"
        p <- ggplot2::ggplot(
            #data = df, ggplot2::aes_string(x=input$x_axis, y=input$y_axis, color=input$colorBy)
            data = df, ggplot2::aes(x_axis, y_axis, color=Dcolor)
