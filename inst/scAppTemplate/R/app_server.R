...
app_server <- function(input, output, session) {
    ...
    output$multiExperiment <- reactive({
      if (nrow(dfkey) > 1){
        multiExperiment <- TRUE
      } else {
        multiExperiment <- FALSE
      }
      ...
      if (length(dropDownList) > 0){
        ...
        output$dropDownPanel = renderUI({
          ...
          dropdown_list <- lapply(1:length(dropDownList), function(i) {
            ...
            names(selOptions) <- as.vector(dropDownList[[displayID]][["selDisplayOptions"]])#as.vector(dropDownList[[displayID]][["selDisplayOptions"]])
